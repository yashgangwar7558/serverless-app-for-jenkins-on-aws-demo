def bucket = 'serverless-app-deployment-packages-for-jenkins-on-aws-demo'
def functionName = 'ServerlessApp-JenkinsOnAWS'
...
node('workers'){
    ...
    stage('Build'){
        sh """
            docker build -f Dockerfile.build -t serverless-app-for-jenkins-on-aws-demo .
            containerName=\$(docker run -d serverless-app-for-jenkins-on-aws-demo)
            docker cp \$containerName:/serverless-app-for-jenkins-on-aws-demo/main main
